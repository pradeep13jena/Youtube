...
export default function Comments({comment, videos, token, setComments, user}) {
  ...
  const editComment = (commentId, videoid, editedText, token) => {
    ...
    if (commentId && videoid && editedText && token) {
      ...
      alert('Please provide all required inputs (commentId, videoid, editedText, and token).');
    }
  };
  ...
  return (
      ...
      >
            ...
            {editingCommentId === comment._id ? (
                <div className="flex flex-col flex-1 gap-2">
                  ...
                  <div className={`flex gap-3`}>
                    ...
                    <button
                      onClick={() => editComment(comment._id, videos._id, editedText, token)}
        ...
        <div>
          <div onClick={handleClick} className={`${comment.username !== user.username ? "hidden" : ""} cursor-pointer transition duration-150 hover:bg-gray-400 w-10 h-10 flex justify-center items-center rounded-full`}>
            <MoreVertIcon />
          </div>
          <Popper id={id} open={open} anchorEl={anchorEl}>
            <Box sx={style}>
              <div  onClick={() => {deleteComment(comment._id); handleClose}} className="flex items-center gap-3 cursor-pointer hover:rounded-md hover:bg-gray-300 px-3 py-3">
                <DeleteOutlineIcon />
                <p className="font-roboto text-base text-black hover:rounded-md">
                  Delete Comment
                </p>
              </div>
              <div onClick={() => {handleEditClick(comment._id); handleClose}} className="flex items-center gap-3 cursor-pointer hover:rounded-md hover:bg-gray-300 px-3 py-3">
                <EditIcon />
                <p className="font-roboto text-base text-black hover:rounded-md">
                  Edit Comment
                </p>
