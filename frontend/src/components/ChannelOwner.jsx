...
export default function ChannelOwner() {
  ...
  function deleteChannel(channel){
    const confirmDelete = window.confirm(`Are you sure you want to delete ${channel}?`)
    if(confirmDelete){
  ...
  function handleVideoValidation(values) {
    ...
    if (!values.videoTitle) {
        ...
        errors.videoTitle = "Video Title must only contain alphanumeric characters and spaces!";
    }
    ...
    if (!values.videoURL) {
        ...
        errors.videoURL = "Video URL must be a valid URL starting with http/https!";
    }

    if (!values.videoThumbnail) {
        errors.videoThumbnail = "Video Thumbnail is required";
    } else if (!urlReg.test(values.videoThumbnail)) {
        errors.videoThumbnail = "Thumbnail URL must be a valid URL starting with http/https!";
    }

    if (!values.videoGenre) {
        errors.videoGenre = "Video Genre is required";
    } else if (!genreReg.test(values.videoGenre)) {
        errors.videoGenre = "Video Genre must only contain alphabets, spaces, and commas!";
    }
  ...
  function handleVideoSubmit(e){
    ...
    if(isValid){
      const { videoTitle: title, videoDescription: description, videoURL: videoLink, videoThumbnail: thumbnail, videoGenre: categories } = videoValues;
      const formData = { title, description, videoLink, thumbnail, categories };
      axios.post(`http://localhost:5000/channel/${channelDetails.channelName}/videos`,
        formData,
        {
          headers: {
            Authorization: `JWT ${token}`,
  ...
  function handleBannerValidata(values){
    ...
    if(!values.bannerURL){
      ...
      errors.bannerURL = "Channel Banner URL must be a valid URL starting with http/https!"
    }
  ...
  function handleBannerSubmit(e){
    ...
    if(isValid){
      ...
      axios.put(`http://localhost:5000/channel/${channelDetails.channelName}`, 
      formData,
      {
        headers: {
          Authorization: `JWT ${token}`,
  ...
  function handleLogoValidate(values){
    ...
    if(!values.logoURL){
      ...
      errors.logoURL = "Logo Banner URL must be a valid URL starting with http/https!"
    }
  ...
  function handleNameValidate(values){
    ...
    if (values.channelName) {
      if (!nameReg.test(values.channelName)) {
        errors.channelName = "Letters, numbers, and spaces are allowed and must be between 2 and 15 characters!";
      }
    }
    ...
    <div className="md:h-[calc(100vh-59.2px)] overflow-y-auto px-3 md:px-24 flex flex-col gap-4 md:gap-7 py-0 w-full">
      {!token ? (
          ...
          <p onClick={handleupdateBanner} className="absolute right-2 bottom-2 sm:right-1 sm:bottom-1 cursor-pointer bg-black rounded-full w-[2px] sm:w-9 h-[2px] sm:h-9 p-1 flex justify-center items-center">
            <ModeEditOutlineIcon sx={{color: "white", fontSize: { xs: 15, sm: 15, lg: 20 } }} />
          </p>
          <Modal
              open={updateBanner}
            ...
            <Box sx={style1}>
              <div className=' '>
                <div className='flex mb-8 justify-between items-center'>
                  ...
                  <CloseIcon onClick={() => {setUpdateBanner(false); setBannerError({}); setBannerValues({})}} className='cursor-pointer' sx={{fontWeight: 800, fontSize: 30}}/>
                </div>
                  <form onSubmit={handleBannerSubmit} className='flex flex-col gap-4 items-center justify-center'>
                    <div className='w-full'>
                      <input className='w-full px-2 py-1 text-black placeholder:text-gray-500 font-roboto text-base rounded-md outline-none border-black border-2' type="text" placeholder='Banner URL' name="bannerURL" value={BannerValues.bannerURL} onChange={handleBannerChange}/>
                      <p className='text-red-500 mt-0 text-sm'>{BannerError.bannerURL}</p>
                    </div>
            ...
            <div className="channelDetails w-full flex justify-start items-center gap-4 mb-5 md:mb-0">
                    ...
                    <Box sx={style1}>
                      <div className=' '>
                        <div className='flex mb-8 justify-between items-center'>
                          ...
                          <CloseIcon onClick={() => {setUpdateLogo(false); setLogoError({}); setLogoValues({})}} className='cursor-pointer' sx={{fontWeight: 800, fontSize: 30}}/>
                        </div>
                        <form onSubmit={handleLogoSubmit} className='flex flex-col gap-4 items-center justify-center'>
                          <div className='w-full'>
                            <input className='w-full px-2 py-1 text-black placeholder:text-gray-500 font-roboto text-base rounded-md outline-none border-black border-2' type="text" placeholder='Logo URL' name="logoURL" value={LogoValues.logoURL} onChange={handleLogoChange}/>
                            <p className='text-red-500 mt-0 text-sm'>{LogoError.logoURL}</p>
                          </div>
                          <button type="submit" className='w-full px-2 py-1 font-roboto text-base rounded-md outline-none border-2 border-blue-600'>Update Logo</button>
                        </form> 
                      </div>
                    </Box>
                  </Modal>
              ...
              <div className="flex flex-col md:gap-3">
                <h1 className="text-xl md:text-3xl font-semibold font-roboto">
                  ...
                  <span className="text-xs font-normal cursor-pointer text-blue-600 underline ml-2" onClick={handleEditDescp}>edit</span>
                </h1>
                <Modal
                      open={editDescp}
                    ...
                    <Box sx={style1}>
                      <div className=' '>
                        <div className='flex mb-8 justify-between items-center'>
                          <h1 className='text-base md:text-2xl font-roboto font-semibold'>Edit text</h1>
                          <CloseIcon onClick={() => setEditDescp(false)} className='cursor-pointer' sx={{fontWeight: 800, fontSize: 30}}/>
                        </div>
                        <form action="" className='flex flex-col gap-5' onSubmit={handleNameSubmit}>
                          <div className='w-full'>
                            <input className='w-full px-3 py-1 font-roboto text-lg border-gray-600 rounded-md outline-none border-2' type="text" placeholder='Channel Name' name="newChannelName" value={DescValues.newChannelName} onChange={handleNameChange}/>
                            <p className='text-red-500 mt-0 text-sm'>{DescError.newChannelName}</p>
                          </div>
                          <div className='w-full'>
                    ...
                    <Box sx={style1}>
                      <div className=' '>
                        ...
                        <form onSubmit={handleVideoSubmit} className='flex flex-col gap-5'>
                          <div className='w-full'>
                            <input className='w-full px-3 py-1 font-roboto text-lg border-gray-600 rounded-md outline-none border-2' type="text" placeholder='Video title' value={videoValues.videoTitle} name="videoTitle" onChange={handleVideoChange}/>
                            <p className='text-red-500 mt-0 text-sm'>{videoErrors.videoTitle}</p>
                          </div>
                          <div className='w-full'>
                            <input className='w-full px-3 py-1 font-roboto text-lg border-gray-600 rounded-md outline-none border-2' type="text" placeholder='Video thumbnail'value={videoValues.videoThumbnail} name="videoThumbnail" onChange={handleVideoChange}/>
                            <p className='text-red-500 mt-0 text-sm'>{videoErrors.videoThumbnail}</p>
                          </div>
                          <div className='w-full'>
                            <input className='w-full px-3 py-1 font-roboto text-lg border-gray-600 rounded-md outline-none border-2' type="text" placeholder='Video URL' value={videoValues.videoURL} name="videoURL" onChange={handleVideoChange}/>
                          ...
                          <div className='w-full'>
                            <textarea className='w-full px-3 py-1 font-roboto text-lg border-gray-600 rounded-md outline-none border-2 resize-none'  placeholder='Video Description'  value={videoValues.videoDescription} name="videoDescription" onChange={handleVideoChange}></textarea>
                            <p className='text-red-500 mt-0 text-sm'>{videoErrors.videoDescription}</p>
                          </div>
                          <button type="submit" className='px-3 py-1 font-roboto text-base rounded-md outline-none border-2 border-blue-600'>Add video</button>
                        </form>
                      </div>
                    </Box>
                  </Modal>
