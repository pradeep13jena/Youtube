...
const ShowPLaylist = ({cat, cata, token}) => {
  const [isChecked, setChecked] = useState(cata.videos.some(video => video._id === cat._id))
  ...
  return (
    <div className="flex w-full items-center justify-start gap-6">
      <input checked={isChecked} className="w-4 h-4" type="checkbox" value={cata.name} name={cata.name} id={cata.name} onChange={handleChange}/>
