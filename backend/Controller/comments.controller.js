...
export const editComment = async (req, res) => {
  ...
  try {
    ...
    if (comment.username !== username) {
      return res.status(403).json({ message: 'You can edit your own comments only' });
    }
...
export const deleteComment = async (req, res) => {
  ...
  try {
    ...
    // Check if the comment belongs to the logged-in user
    if (comment.username !== username) {
      return res.status(403).json({ message: 'You can delete your own comments only' });
    }
    ...
    res.status(200).json({ message: 'Comment deleted successfully', comments: video.comments });
  } catch (error) {
    res.status(500).json({ message: 'Something went wrong', error: error.message });
  }
};
